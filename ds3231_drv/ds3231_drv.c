...
#define TEMPLATE "ds3231_drv" /* Name des Moduls */
...
static int __init mydriver_init( void )
{
	if (alloc_chrdev_region(&template_dev_number,0,1,TEMPLATE)<0)
		return -EIO;
	printk("Treiber %s geladen. Reservierte Nummer: %d", TEMPLATE, template_dev_number);
...
static void __exit mydriver_exit( void )
{
	...
	unregister_chrdev_region( template_dev_number, 1 );
	printk("Treiber %s abgemeldet.", TEMPLATE);
	return;
}
